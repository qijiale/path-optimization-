...
logging.basicConfig()
logg = logging.getLogger("CoreTaint.py")
logg.setLevel("DEBUG")
...
class CoreTaint:
    ...
    def __init__(self, p, interfunction_level=0, log_path='/tmp/coretaint.out', exploration_strategy=None,
        ...
        if exploration_strategy is not None and (shuffle_sat or reverse_sat):
            logg.warning("Exploration strategy takes precedence over state shuffling/reversing")
	...
	self._interfunction_level = 5
        ...
        self._branch = {} # 记录分支节点
        global startFun
        self._startFun= startFun # 开始函数
        self._startfunSaddr = self._startFun.addr
        self._startfunEaddr = self._startFun.addr + self._startFun.size

        ###############################

        if logger_obj: # 这是从别的文件中传过来的log对象，不用他
            log = logger_obj

        if type(logg) == logging.Logger:
    ...
    def handler(self, _, frame):
        ...
        """
        # ipdb.set_trace()
        logg.info("Timeout triggered, %s left...." % str(self._force_exit_after))
        ...
        if self._force_exit_after <= 0 and not self._keep_run:
            ...
            if 'z3' in frame.f_code.co_filename:
                logg.info("Hard Timeout triggered, but we are in z3, trying again in 30 seconds")
                signal.alarm(30)
            else:
                logg.info("Hard Timeout triggered, %s left...." % str(self._force_exit_after))
        ...
        * every symbolic leaf of an ast node is concretized to unique value, according on its name.
        ...
        def get_key_cnt(x):
            ...
            if '_' in str(x) and not self.is_tainted(x):
                ...
                if not idx.isdigit():
                    logg.error("get_key_cnt: Symbolic ID parsing failed, using the whole id: %s" % ret)
                    return ret
    ...
    def apply_taint(self, current_path, addr, taint_id, bit_size=None):
        ...
        self.get_state(current_path).memory.store(addr, t) # 将taint t=<BV40 Reverse(taint_buf_r1__0_32)> 放到地址处<BV32 0xf35b4>
        self._restore_taint_flags()
    ...
    def is_or_points_to_tainted_data(self, x, path, unconstrained=False):
        ...
        return self.is_tainted(x, path=path, unconstrained=unconstrained) or \
               self.is_tainted(self.safe_load(path, x, unconstrained=unconstrained), path=path,
                               unconstrained=unconstrained) # 变量中保存的内容是否污点数据
	...
	debug=False
        ...
        # 求解参数的个数,函数参数有寄存器保存 set_regs = ['r0', 'r1', 'r2', 'r3'] 说明这个函数中有4个参数
        while True:
            if index >= len(puts):
                break
        ...
        for r in set_regs:
            reg_cnt = getattr(self.get_state(suc_path).regs, r) # 获取关于r的属性
            # check if it is pointing to a tainted location 检查reg_cnt是否指向了一个污染的位置 self.is_tainted()
            ...
            try:
                ...
                logg.warning("Tried to defererence a non pointer!")
                continue
    ...
    def _vex_messed_up(self, current_path, next_path):
        ...
        if next_path_addr == self._next_inst(bl):
            logg.warning(" VEX fucked up big time!")
            return True
        return False
    ...
    def _flat_explore(self, current_path, check_path_fun, guards_info, current_depth, **kwargs):
        ...
        if not self._keep_run:
            logg.debug("Backtracking due to stop")
            return
      
        current_path_addr = self.get_addr(current_path)
        try:
            logg.debug("%s: Analyzing block %s", self._p.filename.split('/')[-1], hex(current_path_addr))
        except:
            return

        if not self._check_sat_state(current_path, guards_info) and not self._timeout_triggered:
            logg.error("State got messed up!")
        ...
        try:
            ...
            if not self._keep_run:
                return
            logg.error("'Function check path errored out: %s" % str(e))

        try:
            succ_path = current_path.copy(copy_states=True).step() # step() explore successor
        except Exception as e:
            logg.error("ERROR: %s" % str(e))
            return
        ...
        if len(succ_states_unsat) >0:
            if hex(current_path.active[0].addr) not in self._branch: 
                self._branch[hex(current_path.active[0].addr)] = set()
        if len(succ_path.active) >1:
            if hex(current_path.active[0].addr) not in self._branch: 
                self._branch[hex(current_path.active[0].addr)] = set()

        if succ_path.deadended and not succ_states_sat and not succ_states_unsat:
            logg.debug("Backtracking from dead path")
            return
        ...
        for next_state in succ_states:
            if hex(current_path_addr) in self._branch:
                if len(self._branch[hex(current_path_addr)]) >=2:
                    continue
                else:
                    self._branch[hex(current_path_addr)].add(hex(next_state.addr)) 
            ...
            if hasattr(next_state.ip, 'symbolic') and next_state.ip.symbolic:
                if next_state.sat:
                    logg.error("Next state UNSAT")
                logg.warning("Got a symbolic IP, perhaps a non-handled switch statement? FIX ME... ")
                continue
            ...
            try: 
                if (self.get_state(next_path).history.jumpkind == 'Ijk_Call' and not self._vex_messed_up(current_path, next_path)):
                    if not self._is_summarized(current_path, next_path, current_depth):
                        ...
                        if not self._follow_call(current_path, next_path, current_depth): 
                            ...
                            if not any(s.history.jumpkind == "Ijk_FakeRet" for s in succ_states):
                                ...
                                ret_func = getattr(self.get_state(current_path).regs, link_reg)
                                next_path = self._set_fake_ret_succ(current_path, state, ret_addr, ret_func)# 更新了next_path
                            else
                                continue
                        else:
                            self._ljtest = 1
                            if self._ljtest and self.get_addr(current_path) not in followTarget and len(followTarget) >0: 
                                bl = self._get_bb(current_path_addr)
                                if bl.capstone.insns[-1].mnemonic == 'bl': 
                                    cfuaddr = bl.capstone.insns[-1].address 
                                    followlast = sorted(followTarget)[-1]  
                                    if cfuaddr > followlast:
                                        continue
                                ...
                                for i in range(nargs):
                                    ...
                                    if self.is_or_points_to_tainted_data(val_arg, next_path):
                                        try:
                                            ...
                                            if (callNextIns.insn.insn_name()=='str' and callNextIns.insn.op_str.startswith('r0')) or (callNextIns.insn.insn_name()=='mov' and callNextIns.insn.op_str.endswith('r0')) or (callNextIns.insn.insn_name()=='sub' and callNextIns.insn.op_str.endswith('r0, #0')): 
                    ...
                    if next_path.active[0].addr >= self._p.loader.main_object.min_addr and next_path.active[0].addr <= self._p.loader.main_object.max_addr:
                        if len(followTarget) == 0: 
                            if next_path.active[0].addr >= self._startfunSaddr and next_path.active[0].addr <= self._startfunEaddr:
                                maxsinkaddr = sorted(self._sinkTarget)[-1]
                                if next_path.active[0].addr > maxsinkaddr:
                                    continue
                        else:
                            startfun_sinks = [] 
                            for sinkaddr in self._sinkTarget:
                                if sinkaddr >= self._startfunSaddr and sinkaddr <= self._startfunEaddr:
                                    startfun_sinks.append(sinkaddr)
                            if len(startfun_sinks) > 0:
                                MaxStfunSinkaddr = sorted(startfun_sinks)[-1] 
                                if next_path.active[0].addr > MaxStfunSinkaddr:
                                    continue
                            else:
                                maxfollowaddr = sorted(followTarget)[-1]
                                if next_path.active[0].addr > maxfollowaddr:
                                    continue
            except Exception as e:
                logg.error("ERROR: %s" % str(e))
                return
            ...
            logg.info ("Back to block %s", hex(self.get_addr(current_path)))
            if len(succ_states) >1:
                self._pathnums += 1
                paths +=1
            self._new_path = True

        # logg.debug("Backtracking")
        logg.info("Backtracking------------------------------------------------")
    ...
    def flat_explore(self, state, check_path_fun, guards_info, force_thumb=False, **kwargs):
        ...
        self._ljtest = 1 # 1表示优化的，0表示SaTC的
        if force_thumb:
    ...
    def stop_logging(self):
        ...
        if self._default_log:
            logg.info("Done.")
            logg.info("Results in " + self._fp.name)
        self._fp.close()
    ...
    def run(self, state, sinks_info, sources_info, summarized_f=None, init_bss=True,
            check_func=None, force_thumb=False, use_smart_concretization=True,sinksTarget=None,path=None):
        ...
        self._sinkTarget = sinksTarget

        self._pathnums = path # lj add 记录探索的path个数
        ...
        try:
            ...
            logg.warning("Hard timeout triggered")

        if self._timeout_triggered:
            self.logg("\nTimed out...\n")
            logg.debug("Timeout triggered")
